...
class CangJieImpl(val mmkv: MMKV) {

    companion object {

        private val HERE_IMPL_MAP: MutableMap<String, WeakReference<CangJieImpl>> = mutableMapOf()

        @JvmStatic
        fun bucket(name: String, config: Config): CangJieImpl {
            synchronized(CangJieImpl::class) {
                var hereRef = HERE_IMPL_MAP[name]
                var here = hereRef?.get()
                if (here == null) {
                    here = CangJieImpl(createMMKV(name, config))
     ...
     * 存储数据
     */
    fun <E> put(key: String, value: E): CangJieImpl {
        mmkv.apply {
            when (value) {
